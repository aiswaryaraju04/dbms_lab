...
6.SELECT ITEMS.ITEMID FROM ITEMS JOIN ORDERS ON ITEMS.ITEMID=ORDERS.ITEMID MINUS (SELECT ITEMS.ITEMID FROM ITEMS JOIN ORDERS ON ITEMS.ITEMID=ORDERS.ITEMID JOIN DELIVERY ON DELIVERY.CUSTID=ORDERS.CUSTID);
7. SELECT CUSTOMERS.custname from customers INNER JOIN DELIVERY ON DELIVERY.CUSTID=CUSTOMERS.CUSTID;
8. SELECT CUSTOMERS.CUSTNAME FROM CUSTOMERS JOIN ORDERS ON CUSTOMERS.CUSTID=ORDERS.CUSTID MINUS (SELECT CUSTOMERS.CUSTNAME FROM CUSTOMERS JOIN ORDERS ON CUSTOMERS.CUSTID=ORDERS.CUSTID JOIN DELIVERY ON DELIVERY.CUSTID=ORDERS.CUSTID);
9. SELECT CUSTOMERS.CUSTNAME FROM CUSTOMERS INNER JOIN ORDERS ON ORDERS.CUSTID=CUSTOMERS.CUSTID WHERE ORDERS.QUANTITY=(SELECT MAX(ORDERS.QUANTITY) FROM ORDERS);
10. SELECT * FROM CUSTOMERS INNER JOIN ORDERS ON ORDERS.CUSTID=CUSTOMERS.CUSTID JOIN ITEMS ON ITEMS.ITEMID=ORDERS.ITEMID WHERE ITEMS.PRICE >5000;
11. SELECT CUSTOMERS.CUSTNAME,CUSTOMERS.ADDRESS FROM CUSTOMERS INNER JOIN ORDERS ON ORDERS.CUSTID=CUSTOMERS.CUSTID JOIN ITEMS ON ITEMS.ITEMID=ORDERS.ITEMID WHERE ITEMS.ITEMNAME NOT LIKE '%Samsung GalaxyS4%';
12. SELECT * FROM CUSTOMERS RIGHT JOIN ORDERS ON CUSTOMERS.CUSTID=ORDERS.CUSTID;
   SELECT * FROM CUSTOMERS LEFT JOIN ORDERS ON CUSTOMERS.CUSTID=ORDERS.CUSTID;
13.SELECT * FROM CUSTOMERS ORDER BY STATE;
14.SELECT * FROM ITEMS WHERE PRICE >(SELECT AVG(PRICE)FROM ITEMS) ORDER BY ICATEGORY;
